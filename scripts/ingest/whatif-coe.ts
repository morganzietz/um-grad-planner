/**
 * Bulk-download every College of Engineering What-If PDF from Wolverine Access.
 *
 ...
 *
 * Run: npx tsx scripts/ingest/whatif-coe.ts
...
const PDF_DIR = join(process.cwd(), 'data', 'whatif-pdfs', 'coe');
...
function baseMajorName(label: string): string {
  return label
    .replace(/\s+(BSE|BS Engin|BSE\/BS|BA\/BS|AB\/BS|AB or BS|BS\/BA|BSChem|BS|BA|AB)\s*$/i, '')
    .trim();
}
...
/** An Area of Study select currently showing ENGINEERING majors. */
function isEngineeringAreaSelect(info: SelectInfo): boolean {
  const labels = info.options.map((o) => o.label);
  const bseCount = labels.filter((l) => /\bBSE\b\s*$/.test(l)).length;
  return bseCount >= 3 || labels.some((l) => /Aerospace Engineering/i.test(l));
}
...
async function findEngineeringAreaCtx(page: Page): Promise<{ ctx: Ctx; info: SelectInfo } | null> {
  let contexts: Ctx[];
  try { contexts = [page, ...page.frames()]; } catch { contexts = [page]; }
  for (const ctx of contexts) {
    try {
      const selects = await readAllSelects(ctx);
      const area = selects.find((s) => isAreaSelect(s) && isEngineeringAreaSelect(s));
...
async function findScenarioCtx(page: Page): Promise<Ctx | null> {
  ...
  for (const ctx of contexts) {
    try {
      ...
      if (
        selects.some((s) =>
          s.options.some((o) => /engin/i.test(o.label) && o.label.length < 60),
        )
      ) {
        return ctx;
      }
 ...
 */
async function autoSelectEngineering(page: Page): Promise<ScenarioSelection[] | null> {
  const made: ScenarioSelection[] = [];
  for (let round = 0; round < 6; round++) {
    const found = await findEngineeringAreaCtx(page);
    ...
    for (const s of selects) {
      ...
      if (/engin/i.test(s.selectedLabel)) continue; // already engineering
      const engOption = s.options.find(
        (o) =>
          o.value &&
          /engin/i.test(o.label) &&
          !/biomed eng|prosthodontics/i.test(o.label),
      );
  ...
  return (await findEngineeringAreaCtx(page)) ? made : null;
}
...
async function replayScenario(page: Page, selections: ScenarioSelection[]): Promise<boolean> {
  for (let round = 0; round < 4; round++) {
    if (await findEngineeringAreaCtx(page)) return true;
    const ctx = await findScenarioCtx(page);
    if (!ctx) { await sleep(1500); continue; }
  ...
  const auto = await autoSelectEngineering(page);
  return auto !== null;
}
...
async function main() {
  ...
  console.log('      the College of Engineering by itself; if it cannot,');
  ...
  let selections = (await autoSelectEngineering(page)) ?? [];
  let engineering = await findEngineeringAreaCtx(page);
  if (!engineering) {
    ...
    while (Date.now() - t1 < 15 * 60_000) {
      engineering = await findEngineeringAreaCtx(page);
  ...
  for (const opt of targets) {
    ...
    try {
      if (!(await findEngineeringAreaCtx(page))) {
